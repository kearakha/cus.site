...
const serviceOutputSchema = z.object({
  ...
  description: z
    ...
    .min(20)
    .max(400)
    .describe('Deskripsi persuasif 1-3 kalimat (max 400 char), tanpa placeholder'),
});
...
export const kontenAISchema = z.object({
  ...
  aboutParagraph: z
    ...
    .min(80)
    .max(500)
    .describe('Paragraf tentang bisnis, tone sesuai vibe, sebut lokasi spesifik'),
  ctaText: z
    .string()
    .min(2)
    .max(25)
  ...
  accentColor: z
    ...
    .describe('Warna aksen hex (tanpa #). Casual=warm, Professional=blue, Elegant=gold/earth'),
  services: z
...
function buildSystemPrompt(input: WizardInput, mode: 'structured' | 'json-only'): string {
  const template = mode === 'structured' ? SYSTEM_PROMPT_STRUCTURED : SYSTEM_PROMPT_JSON_ONLY;
  ...
  return template
    ...
    .replace(
      '{LAYANAN_LIST}',
      input.layanan.map((l, i) => `${i + 1}. ${l.title}: ${l.description}`).join('\n'),
    );
}

function buildUserPrompt(input: WizardInput): string {
  const layananList = input.layanan
    .map((l, i) => `${i + 1}. title: "${l.title}" | description user: "${l.description}"`)
    .join('\n');
...
export type GenerateKontenResult =
  ...
  | { success: false; error: string; rawResponse?: string; cleanedPreview?: string };
...
export async function generateCusWebsite(
  ...
  if (!process.env.OPENAI_API_KEY) {
    return { success: false, error: 'OPENAI_API_KEY belum di-set di .env.local' };
  }
  ...
  console.warn('[ai] Structured Outputs not supported, falling back to JSON prompt mode');
...
async function tryStructuredOutput(input: WizardInput): Promise<AttemptResult> {
  try {
    ...
    if (!message) return { ok: false, error: 'AI return empty response', shouldFallback: false };

    if (message.refusal) {
      return { ok: false, error: `AI menolak: ${message.refusal}`, shouldFallback: false };
    }
...
async function tryJsonOnlyMode(input: WizardInput): Promise<GenerateKontenResult> {
  try {
...
function extractAndParseJson(
  raw: string,
): { success: true; data: KontenAI } | { success: false; error: string; cleanedPreview?: string } {
...
function validateOutput(parsed: KontenAI, input: WizardInput): AttemptResult {
  ...
  const placeholderRegex = /(lorem ipsum|\[alamat\]|\[nama\]|\[no hp\]|xxx|placeholder|contoh:)/i;
  const allText = [
    parsed.heroHeadline,
    parsed.heroSubtext,
